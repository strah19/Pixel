...
namespace Pixel {
	...
	int Spritesheet::CalculateSpriteCoordinate(const glm::vec2& pixel_point) {
		if (!texture)
			return -1;
		if (pixel_point.x > texture->get()->GetWidth() || pixel_point.y > texture->get()->GetHeight())
			return -1;

		in_range_coordinates = pixel_point / glm::vec2(texture->get()->GetWidth(), texture->get()->GetHeight());

		return 0;
	}

	glm::vec2* Spritesheet::CalculateRectCoordinates(const glm::vec2& pixel_point, const glm::vec2& sprite_size) {
		float y_coord = texture->get()->GetHeight() - (pixel_point.y + sprite_size.y);
